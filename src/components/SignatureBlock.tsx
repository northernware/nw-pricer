...
export default function SignatureBlock({
  ...
  const signToken = searchParams.get("sign") ?? undefined;
  ...
  const handleApprove = async () => {
    ...
    const res = await approveProjectAction(projectId, signature, signToken);
  ...
  if (isApproved) {
    return (
      <div className="w-full border border-nw-acid bg-nw-acid/5 p-6 relative overflow-hidden">
        ...
        {title && (
          <div className="text-[10px] text-nw-acid font-mono uppercase mb-1">{title}</div>
        )}
        <div className="text-[11px] text-nw-graphite font-mono uppercase">Digitally Signed</div>

        <div className="mt-6 pt-4 border-t border-nw-acid/20 text-[10px] font-mono text-nw-graphite leading-relaxed">
          Timestamp: {approvedAt ? new Date(approvedAt).toLocaleString("en-PH") : "Unknown"}
          <br />
          IP: {ipAddress || "Logged"}
          <br />
          {snapshotHash && (
  ...
  if (!canSign) {
    return (
      <div className="w-full border border-nw-graphite/20 bg-nw-bone/50 p-6">
        <div className="text-[10px] font-bold uppercase track-widest mb-3 text-nw-graphite font-mono">
          View-only link
        </div>
        <p className="text-xs text-nw-graphite leading-relaxed">
          This link lets you review the contract. To sign, use the signing link sent by your account
          manager.
        </p>
      </div>
    );
  ...
  return (
    <div className="w-full">
      ...
      <p className="text-xs text-nw-graphite mb-6 leading-relaxed">
        By typing your name and clicking &quot;Approve & Sign&quot;, you officially agree to the terms,
        scope, and financial investment outlined in this document. This serves as a legally binding
        digital signature.
      </p>
